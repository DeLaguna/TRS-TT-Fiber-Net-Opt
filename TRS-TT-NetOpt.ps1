...
if (!(Test-Path -Path $speedtestPath)) {
    ...
    Expand-Archive -Path "$tempFolder\speedtest.zip" -DestinationPath "." 
    Remove-Item -Path "$tempFolder\speedtest.zip" 
}
$Status = "Speed Test Installed"
# Run speed test before optimization
Write-Host "Running speed test before optimization..." -ForegroundColor Yellow
$beforeSpeedTest = & $speedtestPath --format=json --accept-license --accept-gdpr | ConvertFrom-Json 
...
Write-Host "Speed test before optimization: Download $($beforeDownloadMbps) Mbps, Upload $($beforeUploadMbps) Mbps" -ForegroundColor Red
...
Write-Host "Killed tasks that were not responding" -ForegroundColor Red
...
switch ($choice) {
    '1' { Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" -Name "SystemResponsiveness" -Value 20
          Write-Host "SystemResponsiveness set to 20 for Gaming/Streaming"  }
    '2' { Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" -Name "SystemResponsiveness" -Value 10
          Write-Host "SystemResponsiveness set to 10 for Just Gaming"  }
    '3' { Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" -Name "SystemResponsiveness" -Value 0
          Write-Host "SystemResponsiveness set to 0 for File Transfer"  }
...
Write-Host "Running speed test after optimization..." -ForegroundColor Yellow
$afterSpeedTest = & $speedtestPath --format=json --accept-license --accept-gdpr | ConvertFrom-Json 
...
Write-Host "Speed test after optimization: Download $($afterDownloadMbps) Mbps, Upload $($afterUploadMbps) Mbps" -ForegroundColor Red
...
Write-Host "Speed test before optimization: Download $($beforeDownloadMbps) Mbps, Upload $($beforeUploadMbps) Mbps" -ForegroundColor Red
Write-Host "Speed test after optimization: Download $($afterDownloadMbps) Mbps, Upload $($afterUploadMbps) Mbps" -ForegroundColor Green
